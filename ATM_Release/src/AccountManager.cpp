...
void AccountManager::loadAccounts() {
    ...
    while (std::getline(file, line)) {
        ...
        if (std::getline(iss, accountNumber, ',') &&
            ...
            long long dayLimit = std::stoll(dayLimitStr);
            long long onceLimit = std::stoll(onceLimitStr);
            double dailySpent = std::stod(dailySpentStr);
            int lastTransactionYear = std::stoi(lastTransactionYearStr);
            int lastTransactionMonth = std::stoi(lastTransactionMonthStr);
            int lastTransactionDay = std::stoi(lastTransactionDayStr);
            int failedAttempts = std::stoi(failedAttemptsStr);
            ...
            if (std::getline(iss, transactionCountStr)) {
                int transactionCount = std::stoi(transactionCountStr);
                ...
                for (int i = 0; i < transactionCount; ++i) {
                    ...
                    if (std::getline(file, transactionLine)) {
                        ...
                        if (std::getline(tiss, type, '|') &&
                            ...
                            transaction.setAmount(std::stod(amountStr));
